# CALHN-DataCollection Backlog

Live app: https://jarran-zen-musictherapy.github.io/CALHN-DataCollection/

---

## Open Issues (GitHub)

| # | File | Task | Priority |
|---|------|------|----------|
| [#27](https://github.com/jarran-zen-musictherapy/CALHN-DataCollection/issues/27) | Global | **Bug:** Data will not load if any button selected before data has loaded | High |
| [#26](https://github.com/jarran-zen-musictherapy/CALHN-DataCollection/issues/26) | MTOoS.html | Export & Date range — align on mobile view | Medium |

---

## Upcoming / Untracked Work

> Add tasks here as they come up. Move to GitHub issues once scoped.

- [ ] ...

---

## Recently Completed

| # | Task | Completed |
|---|------|-----------|
| [#25](https://github.com/jarran-zen-musictherapy/CALHN-DataCollection/issues/25) | Index.html — Add VAS Tracker Data Visualiser | 2026-03-25 |
| [#20](https://github.com/jarran-zen-musictherapy/CALHN-DataCollection/issues/20) | Index.html — MT/AT descriptions | 2026-03-25 |
| [#24](https://github.com/jarran-zen-musictherapy/CALHN-DataCollection/issues/24) | Index.html — Add Affect Wheel Data Visualiser | 2026-03-10 |
| [#17](https://github.com/jarran-zen-musictherapy/CALHN-DataCollection/issues/17) | Priority-groups.html — Expand CALD subtitle | 2026-03-10 |
| [#19](https://github.com/jarran-zen-musictherapy/CALHN-DataCollection/issues/19) | Music Therapy OoS — description, reporting periods, PNG/PDF export | 2026-03-10 |
| [#22](https://github.com/jarran-zen-musictherapy/CALHN-DataCollection/issues/22) | Index.html — Remove MT Mood Tracker Pre/Post Analysis | 2026-03-10 |
| [#21](https://github.com/jarran-zen-musictherapy/CALHN-DataCollection/issues/21) | Index.html — Remove 2024 year | 2026-03-10 |
| [#23](https://github.com/jarran-zen-musictherapy/CALHN-DataCollection/issues/23) | MT Mood Tracker — Change 'Occasions of Service' button to 'View' | 2026-03-10 |
| [#31](https://github.com/jarran-zen-musictherapy/CALHN-DataCollection/issues/31) | PWA implementation (manifest, service worker, local-first sync) | 2026-03 |

---

## How to use this file

- **Open Issues** — mirrors GitHub issues; update when issues are opened/closed
- **Upcoming / Untracked Work** — lightweight capture for ideas not yet on GitHub
- **Recently Completed** — running log of what's done for quick context
